...
let currentPhase = 0;
const checked = {};
const findings = {}; // findings[`${pi}-${si}`] = { fields: {}, status: '', savedAt: null }
...
function saveToStorage(pi, si) {
  ...
  try {
    localStorage.setItem(`bizaudit:step:${key}`, JSON.stringify(data));
...
function saveClientName() {
  ...
  try {
    localStorage.setItem('bizaudit:client', name);
  } catch(e) {}
}
...
function loadAllFromStorage() {
  ...
  for (const pi in PHASES) {
    for (const si in PHASES[pi].steps) {
      const key = `${pi}-${si}`;
      try {
...
function stepHasNotes(pi, si) {
  ...
  if (!f || !f.fields) return false;
  return Object.values(f.fields).some(v => v && v.trim() !== '');
}
...
function renderSidebar() {
  ...
  PHASES.forEach((p, pi) => {
    const pd = phaseDone(pi), pt = p.steps.length, pct = Math.round(pd/pt*100);
    const hasNotes = PHASES[pi].steps.some((_, si) => stepHasNotes(pi, si));
    const isA = pi === currentPhase;
    h += `<div class="nav-item${isA?' active':''}" style="--ph-color:${p.color}" onclick="switchPhase(${pi})">
...
function renderMain() {
  ...
  const pd = phaseDone(currentPhase), pt = p.steps.length;
  ...
  p.steps.forEach((s, si) => {
    ...
    const firstNote = f && f.fields ? Object.values(f.fields).find(v => v && v.trim()) : '';
          ...
          <div class="findings-section" id="findings-section-${si}">
            <div class="findings-header">
              ...
              <div class="save-status" id="save-status-${si}">
                <span class="si"></span> <span id="save-text-${si}">Not saved yet</span>
              </div>
            </div>
  ...
  p.steps.forEach((s, si) => {
    ...
    s.fields.forEach(field => {
      const el = document.getElementById(`field-${currentPhase}-${si}-${field.key}`);
      if (!el) return;
      const val = f.fields[field.key] || '';
      if (field.type === 'rating') {
        // rating buttons — handled by JS class
        document.querySelectorAll(`[data-step="${currentPhase}-${si}"][data-field="${field.key}"]`).forEach(btn => {
    ...
    // Update save status if there are saved values
    if (f.savedAt) {
...
function updateNotePreview(pi, si) {
  ...
  if (hasNotes) {
    ...
    if (span && firstNote) span.textContent = firstNote.substring(0, 80) + (firstNote.length > 80 ? '…' : '');
  }
}
...
function toggleCheck(si) {
  ...
  if (checked[key]) { card.classList.add('done'); chk.classList.add('checked'); }
  else { card.classList.remove('done'); chk.classList.remove('checked'); }
...
function getRatingLabel(key, val) {
  if (!val) return null;
  return val;
}
...
function renderSummaryTab() {
  ...
  let h = `
    <div class="summary-grid">
      ...
      <div class="summary-stat"><div class="summary-stat-num">${filled}</div><div class="summary-stat-label">Steps with findings</div></div>
      <div class="summary-stat"><div class="summary-stat-num" style="color:#dc2626">${criticals}</div><div class="summary-stat-label">Critical issues flagged</div></div>
    </div>
    <div style="font-size:13px;color:var(--text-secondary);margin-bottom:16px;padding:10px 14px;background:var(--main-bg);border-radius:6px;border:1px solid var(--border-light)">
      <strong>Client:</strong> ${clientName} &nbsp;·&nbsp; <strong>Audit date:</strong> ${new Date().toLocaleDateString('en-GB', {day:'numeric',month:'long',year:'numeric'})}
    </div>
  `;
...
function renderAllTab() {
  ...
  PHASES.forEach((p, pi) => {
    const phaseStepFindings = PHASES[pi].steps.map((s, si) => ({ s, si, f: findings[`${pi}-${si}`] }))
      .filter(({ f }) => f && f.fields && Object.values(f.fields).some(v => v && v.trim()));

    if (phaseStepFindings.length === 0) return;
    hasAny = true;
    ...
    phaseStepFindings.forEach(({ s, si, f }) => {
      h += `<div class="finding-entry">
        <div class="finding-entry-step">${String(si+1).padStart(2,'0')} — ${s.name}</div>`;
      ...
      s.fields.forEach(field => {
        ...
        if (field.type === 'rating') {
          const colors = { 'critical': 'critical', 'needs-work': 'needs-work', 'good': 'good' };
          const label = field.labels[field.options.indexOf(val)] || val;
          h += `<span class="finding-entry-rating ${colors[val] || ''}">${field.label}: ${label}</span> `;
        } else {
          h += `<div style="margin-top:6px"><div style="font-size:10px;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px">${field.label}</div>
            <div class="finding-entry-notes" style="white-space:pre-wrap">${val}</div></div>`;
        }
      });
  ...
  if (!hasAny) {
    h = `<div class="empty-state">
      <svg width="40" height="40" viewBox="0 0 40 40" fill="none"><rect x="8" y="6" width="24" height="28" rx="2" stroke="#8c959f" stroke-width="1.5"/><path d="M14 14h12M14 19h12M14 24h7" stroke="#8c959f" stroke-width="1.5" stroke-linecap="round"/></svg>
...
function renderExportTab() {
  ...
  let h = `
    <div style="font-size:13px;color:var(--text-secondary);margin-bottom:14px;line-height:1.6">
      This structured JSON contains all your findings and is ready for the report generator. Copy it to use with any AI tool, document builder, or Firebase import.
    </div>
    <div class="export-code" id="export-json">${json.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
    <div class="export-actions">
      <button class="btn-copy" id="copy-btn" onclick="copyExport()">📋 Copy JSON</button>
      <button class="btn-copy" onclick="downloadExport('${clientName.replace(/[^a-z0-9]/gi,'_')}')">⬇ Download .json</button>
    </div>

  `;

  document.getElementById('tab-export').innerHTML = h;
}
...
function copyExport() {
  ...
  navigator.clipboard.writeText(text).then(() => {
    ...
    if (btn) { btn.textContent = '✓ Copied!'; btn.classList.add('success'); setTimeout(() => { btn.textContent = '📋 Copy JSON'; btn.classList.remove('success'); }, 2000); }
